...
def optimize_coupon(
    ...
    """Bütçe kısıtı altında P(hepsi doğru) değerini maksimize eden kuponu döner.
    ...
    return _optimize_rows(
        _normalize(predictions), max_columns, budget, column_price, max_triples
    )
...
def _optimize_rows(
    ...
    selections = _backtrack(rows, choice, picks_per_match, doubles, triples)
    ...
    return CouponPlan(
        ...
        doubles=doubles,
        triples=triples,
        p_all_correct=math.exp(best[0]),
        column_price=column_price,
        distribution=correct_count_distribution(covers),
    )


def _backtrack(rows, choice, picks_per_match, doubles: int, triples: int) -> list[MatchSelection]:
    """DP kararlarını geriye izleyerek maç bazında seçimleri kurar."""
    n = len(rows)
    a, b = doubles, triples
    sizes = [0] * n
